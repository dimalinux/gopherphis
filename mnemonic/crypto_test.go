...
func TestPrivateKeyPairToAddress(t *testing.T) {
	...
	require.NoError(t, err)
	address := kp.PublicKeyPair().Address(Mainnet)
...
func TestKeccak256(t *testing.T) {
	...
	res2 := ethcrypto.Keccak256([]byte{1})
...
func TestPrivateSpendKey_View(t *testing.T) {
	...
	for _, tt := range tests {
		spendKey, err := hex.DecodeString(tt.spendKey)
		require.NoError(t, err)
		skScalar, err := edwards25519.NewScalar().SetCanonicalBytes(spendKey)
		require.NoError(t, err)
		psk := &PrivateSpendKey{
			skScalar,
		}
